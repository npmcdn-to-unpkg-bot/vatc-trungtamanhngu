...
class HqSystemTool {
    ...
    static public function copyRecursive($source, $dest, $statFiles = false, $excludeNames = [])
    {
        ...
        $it            = new \RecursiveIteratorIterator(
                new \RecursiveDirectoryIterator($source, $iteratorFlags), \RecursiveIteratorIterator::SELF_FIRST
        );
     ...
     * @throws Exception
     */
    static public function rmdirRecursive($path, $includeSelf = false)
    {
        $it = new \RecursiveIteratorIterator(
                new \RecursiveDirectoryIterator($path, \RecursiveDirectoryIterator::KEY_AS_PATHNAME), \RecursiveIteratorIterator::CHILD_FIRST
        );
     ...
     * @return array
     */
    static public function recursiveGlob($defaultPath = '', $pattern = '*', $flags = 0)
    {
        ...
        {
            $files = array_merge($files, self::recursiveGlob($path, $pattern, $flags));
        }
